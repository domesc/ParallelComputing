...
class KMeans {
  ...
  /**
    * Step 0: classify the points by their mean
    */
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if(!points.isEmpty)
      points.groupBy(p => findClosest(p, means))
    else
      means.map(m => (m, List())).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(oldMean => findAverage(oldMean, classified.get(oldMean).getOrElse(List())))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    if (oldMeans.size !=  newMeans.size)
