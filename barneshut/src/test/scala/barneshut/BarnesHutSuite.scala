...
class BarnesHutSuite extends FunSuite with Matchers {
...
import FloatOps._
  ...
  test("Leaf.insert(b) should return a new Fork if size > minimumSize and add the bodies to the right quad") {
    val b1 = new Body(123f, 18f, 26f, 0f, 0f)
    val b2 = new Body(123f, 19f, 26f, 0f, 0f)
    val quad = Leaf(17.5f, 27.5f, 5f, Seq(b1))

    val newQuad = quad.insert(b2)

    newQuad shouldBe a [Fork]
    val expectedFork: Fork = newQuad.asInstanceOf[Fork]
    expectedFork.nw shouldBe a [Empty]
    expectedFork.ne shouldBe a [Fork]
    expectedFork.sw shouldBe a [Empty]
    expectedFork.se shouldBe a [Empty]
  }
  ...
  test("Sector matrix combine") {
    ...
    sm2 += body
    val smResult = sm1.combine(sm2)
    val res = smResult(2, 3).size == 1 && smResult(2, 3).find(_ == body).isDefined
    assert(res, s"Body not found in the right sector")
  }
